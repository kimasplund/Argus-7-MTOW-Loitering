...
**And the balance claim fails the programme's own written threshold, not merely mine.** The specification requires **8% ≤ SM ≤ 20% MAC** (`docs/superpowers/specs/2026-08-20-argus7-cad-sim-optimisation-design.md`, line 109). The search that produced v3.0 ran a 5–15% window — a pre-registered threshold moved without anyone noticing, the precise failure pre-registration exists to prevent, now committed on the adopted design point. `scripts/run_optimisation_layout.py` says so in its own header: *"v3.0 landed at +5.79% and duly FAILED the real gate."* Re-runs at the real threshold exist (`opt_runs/layout_sm8_span12.json`), and the `design/argus7_v4.yaml` built from one returns **+12.78% MAC full, +13.72% dry** against the authoritative module. v3.0 is premortemed here because the brief names it current; the reader should know the repository already holds its successor, and holds it because v3.0 failed a stability requirement mode #3 below otherwise has to rediscover from first principles.
...
**Cause of death:** internal layout, wing tooling and layup, fuselage and booms, engine conversion and bench running, avionics and C2, recovery integration and drop test, authorisation paperwork, flight test. My own hour bands for a composite airframe this size total **1,900–3,150 h**. Eighteen months at 20 h/week is 1,560 h — short of even the optimistic end of the band, so the evenings-and-weekends version of this plan does not close on arithmetic alone. At genuine full time it is ~3,120 h, which covers all but the top of the band and leaves nothing at all for rework, and this is a first article in a material where the first part is always remade.
**How it unfolded:**
...
- Oct 2027: the 24 h demonstration flies and passes — but only just, and only low. At 172 kg takeoff (55% of MTOW) on 20 kg of fuel, `simulate_loiter` returns **24.5 h at sea level and 22.7 h at the 4,359 m design altitude**: it clears 24 h by 2%, and only by being flown down where the air is thick. At that weight the climb margin is 5.4 kW of 10.8 and the stall is 61 km/h; every hard constraint on this page is inactive.
...
|---|---|---|
| The propeller cannot absorb its engine (v1.0's killer: C_P 0.911 vs a ~0.25 ceiling) | Held down only because I re-ran it. The propeller record and its BEMT sweep ran on `design/argus7_v2.yaml` — 8.15 kW, 248 kg, 4,191 m — and v2.0 is retired; its 99%-absorption figure is the 1.00 m blade's, its η 0.858 the 1.04 m blade's, so the pair quoted for v3.0 is two propellers on a different aeroplane. Re-run at v3.0's point, the 1.04 m disc absorbs **99.3% of 10.845 kW** and returns **η 0.855** — but at **~2,030 rpm, not the 1,900 the design file records**; at 1,900 it makes 58.1 N against the 78.3 N needed for level flight. The file's `prop_rpm` and 3.947:1 reduction are v2.0's and wrong here. Clearance is fine: 190 mm at 11.27 m span from `derive_booms`; the record's 56 mm is **v1.0**'s 9.26 m span, not v2.0, which clears by 208 mm | Already open, not pending: any dyno-confirmed rating change in either direction needs a fresh BEMT sweep at the real operating point; likewise a boom `y_station_frac` inboard of 0.134 |
...
**The hidden assumption:** by convergence, modes 3, 4, 5 and 7 all die unless one belief holds — **that the optimiser's bounds described what exists**. Two of the eleven variables sit hard against a bound: MTOW 312.93 of a 320 ceiling, `bsfc_full` 0.2519 against a 0.250 floor. Those are exactly the two that carry the fatal flaw. Three more — wing area 5.9191, taper 0.697, t/c 0.1909 — sit within 5% of the *specification's* ranges (`argus7.opt.design_space.Bounds`: area ≤ 6.0, taper ≤ 0.70; t/c ≤ 0.20 in the eight-variable run), but `scripts/run_optimisation_layout.py` today carries area ≤ 12.0, taper ≤ 1.00, t/c ≤ 0.22, under which all three are interior. I cannot establish which set was live when v3.0 was written, so those three are unresolved, not evidence. The two that are certain suffice: v3.0's mass and its fuel burn are both against a wall, and each wall was drawn from an assumption rather than from a catalogue, a scale, or a measurement.

**Fatal flaw:** yes, and it is nameable. **v3.0's 6.33 days is bought with a 250 g/kWh engine that the programme's own research pack records as a verified product gap in this power class, burning a fuel dense enough to fit a tank that only closes at kerosene density.** Resolve the fuel and one of the two collapses. Everything else on this page is expensive; this one is close to arithmetic — with one caveat that cuts each way. The 252 g/kWh full-load figure is measured on nothing; it is what 339 at 30.6% load implies *through the generic Willans coefficients* in `argus7.opt.coupled` (0.8471, 0.1529), a correlation nobody has checked against a 160–250 cc four-stroke. A flatter real curve would let a worse full-load engine deliver 339 effective, and `research/design_pack.md` estimates the budget donor directly at **330–420 g/kWh at 20–40% load [EST]** — making v3.0's 339 the optimistic edge of an estimated band rather than a documented impossibility. Against that: the band's midpoint is 375, worse than the realistic stack above already assumes. Either way the number the aircraft is designed around is an estimate stacked on a correlation, and one dyno afternoon settles both.
...
|---|---|---|---|---|
| 1 budget | wing mould quote, then committed spend | quote >€15k; or >€22k committed with nothing measured | 2026-10-31, 2027-02-28 | freeze tooling until the dyno is bought — and note that dropping to 250 kg does **not** shrink the mould: that point's span is 11.54 m against v3.0's 11.27 m, so the tooling saving has to come from a shorter span chosen deliberately, not from the de-rate |
...
| 3 static margin | installed powertrain mass, weighed; payload centroid on the layout drawing; tank bays still on the 40%-chord centroid | >22 kg; aft of 0.52 m (the dry-state limit); bays moved | 2026-11-30, 2027-01-31 | re-solve `x_le_frac` before the fuselage plug is cut — move the wing, not the ballast |
...
| 5 engine | dyno best-point BSFC on the chosen fuel, **and the mapped part-load point at 30% load** — the endurance case rests on the second, not the first | >280 g/kWh full-load, or >345 g/kWh at 30% load | 2027-02-28 | withdraw the 6-day claim in writing; re-run the BEMT sweep at the real rating before ordering a blade — the existing sweep is v2.0's 8.15 kW and does not cover this engine |
